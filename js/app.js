...
class Aq {
	...
	get getAnswer() {
		return String(this.answer).replace(
			this.link,
			"<a href='#'>" + this.link + '</a>'
		);
	}
...
const initProfileClone = () => {
	...
	const profiles = [
		...
		new SliderProfile(
			'https://storage.googleapis.com/t3n-de/pioneers/8298acd960400f97e749266416913a54ac9b45ea/29513215_1757429684277443_7955459664427213519_n%20-%20Kathrin%20We%C3%9Fling.jpg',
			'Kathrin Weßling',
			'Founder',
			'Würger & wessling'
		)
...
const initSlider = () => {
	const sliderDefaultDiv = SetElement.setElements(
		'section',
		'sliderDefaultDiv'
	);
...
const sliderButtonInit = () => {
	...
	const mousemoveTouchmoveFunction = (e) => {
		if (isClicked && e.type == 'mousemove') {
			sliderDiv.style.transform =
				'translateX(' +
				(-sliderDiv.clientWidth * counter + (-mousedownPosition + e.clientX)) +
				'px)';
			sliderDiv.style.transition = 'none';
		} else if (isClicked && e.type == 'touchmove') {
			sliderDiv.style.transform =
				'translateX(' +
				(-sliderDiv.clientWidth * counter +
					(-mousedownPosition + event.touches[0].clientX)) +
				'px)';
			sliderDiv.style.transition = 'none';
		}
...
const initTeileDeinWissen = () => {
	...
	const imgDiv = SetElement.setElements(
		'div',
		'section3_imgDiv',
		'section3_insideDiv'
	);
	const listDiv = SetElement.setElements(
		'div',
		'section3_listDiv',
		'section3_insideDiv'
	);
	...
	const jsonAttribute = [
		...
		{
			src:
				'https://storage.googleapis.com/t3n-de/pioneers/a0e9cb9419205aeb77d95c7de29244579285f3da/PioneersScreen.png'
		}
	];
...
const initPioniereInListenFinden = () => {
	...
	const textDiv = SetElement.setElements(
		'div',
		'section4_textDiv',
		'section4_insideDiv'
	);
...
const initLassDichInspirieren = () => {
	...
	source.src =
		'https://storage.googleapis.com/t3n-de/pioneers/cfa02d7739eea252d2161f78e8980ed9e8437cfb/PN_Features.mp4';
...
const initChatButton = () => {
	...
	localStorage.getItem('pulse') == null
		? (chatButton.className = 'pulseAnimation')
		: chatButton;
	...
	for (let i = 0; i < count; i++) {
		...
		const svg = SetElement.setElements('svg');
	...
	const svgs = document.querySelectorAll('#chatButton > span > svg');

	button.style.opacity = 0;
	button.style.transform = 'scale(0.1,0.1)';
	setTimeout(() => {
...
const getMovementSection = function() {
	...
	paragraph.innerHTML =
		'Zeig, wer du bist und werde Mitglied im Pioneers Network – dem Netzwerk für Menschen, die die Digitalisierung gestalten. Menschen, die etwas bewegen wollen, die kreativ sind, die eine Haltung haben, die positiv denken, konstruktives Feedback und Impulse geben und Spaß bei der Arbeit haben. Digitale Pioniere eben.';
